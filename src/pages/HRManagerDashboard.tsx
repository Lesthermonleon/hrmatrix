...
import type { Employee, LeaveRequest, AttendanceRecord, LeaveBalance } from '../lib/supabase'
...
export function HRManagerDashboard({ activeSection, onNavigate }: HRProps) {
  ...
  const [showEditBalance, setShowEditBalance] = useState<LeaveBalance | null>(null)
  ...
  async function fetchAll() {
    setLoading(true)
    const [empRes, leaveRes, attRes, balRes] = await Promise.all([
      ...
      supabase.from('leave_balances').select('*, employee:employees(full_name)').order('employee_id'),
    ])
  ...
  async function handleUpdateBalance() {
    ...
    const { error } = await supabase.from('leave_balances').upsert({
      ...
      employee_id: showEditBalance.employee_id,
      leave_type: showEditBalance.leave_type,
      year: showEditBalance.year,
      total_days: showEditBalance.total_days,
      used_days: showEditBalance.used_days,
  ...
  function handleExportCSV() {
    ...
    a.href = url
    a.download = `hr_employees_${new Date().toISOString().split('T')[0]}.csv`
    a.click()
  ...
  return (
    <div className="wrap">
      ...
      {activeSection === 'balances' && (
        <div className="card">
          <div className="card-hd">
            <div><div className="card-title">Leave Balances</div><div className="card-sub">{new Date().getFullYear()} — {leaveBalances.length} records</div></div>
          </div>
          <div className="tbl-wrap">
            <table className="tbl">
              <thead><tr><th>Employee</th><th>Leave Type</th><th>Total</th><th>Used</th><th>Remaining</th><th>Edit</th></tr></thead>
              <tbody>
                {leaveBalances.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--ink3)', padding: 24 }}>No leave balances configured. Balances are created when employees are assigned leave allocations.</td></tr>}
                {leaveBalances.map(b => (
                  <tr key={b.id}>
                    <td style={{ fontWeight: 500 }}>{(b.employee as any)?.full_name || '—'}</td>
                    <td><span className="badge badge-purple">{b.leave_type}</span></td>
                    <td>{b.total_days}d</td>
                    <td>{b.used_days}d</td>
                    <td style={{ fontWeight: 600, color: (b.total_days - b.used_days) > 0 ? 'var(--ok)' : 'var(--danger)' }}>{b.total_days - b.used_days}d</td>
                    <td><button className="btn btn-ghost btn-xs" onClick={() => setShowEditBalance({ ...b })}>Edit</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className={`modal-ov${showEditBalance ? ' active' : ''}`} onClick={e => { if (e.target === e.currentTarget) setShowEditBalance(null) }}>
        <div className="modal-box">
          <div className="modal-hd">
            <div><div className="modal-title">Edit Leave Balance</div></div>
            <button className="modal-x" onClick={() => setShowEditBalance(null)}>✕</button>
          </div>
          {showEditBalance && (
            <div className="modal-body">
              <div className="form-row fr-2">
                <div className="form-grp"><label className="form-lbl">Total Days</label><input className="form-ctrl" type="number" value={showEditBalance.total_days} onChange={e => setShowEditBalance(p => p ? { ...p, total_days: Number(e.target.value) } : p)} /></div>
                <div className="form-grp"><label className="form-lbl">Used Days</label><input className="form-ctrl" type="number" value={showEditBalance.used_days} onChange={e => setShowEditBalance(p => p ? { ...p, used_days: Number(e.target.value) } : p)} /></div>
              </div>
              <button className="btn btn-primary" style={{ width: '100%', marginTop: 8 }} onClick={handleUpdateBalance}>Save Changes</button>
            </div>
          )}
        </div>
      </div>
