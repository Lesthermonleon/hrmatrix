...
export function PayrollOfficerDashboard({ activeSection, onNavigate }: PayrollProps) {
  ...
  async function handleGeneratePayroll(period: PayrollPeriod) {
    ...
    const insertData = employees.map(emp => {
      ...
      empLeaves.forEach(l => {
        const bal = allBalances.find(b => b.employee_id === emp.id && b.leave_type === l.leave_type)
        const remaining = bal ? (bal.total_days - bal.used_days) : 0
        if (remaining <= 0) unpaidLeaveDays += l.days_count
        else if (l.days_count > remaining) unpaidLeaveDays += (l.days_count - remaining)
      })
    ...
    showToast(`Payroll generated for ${employees.length} employees`, '✅')
    fetchAll()
  ...
  async function handleSubmitForReview(id: string) {
    ...
    showToast('Payroll submitted for admin review', '📤')
    fetchAll()
  }
  ...
  async function handleReturnToPayroll(id: string) {
    ...
    showToast('Payroll returned to payroll officer for revision', '🔄')
    fetchAll()
  }
